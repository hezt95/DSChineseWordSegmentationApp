//
//  HZTColor.swift
//  DSChinese
//
//  Created by He Zitong on 15/9/8.
//  Copyright (c) 2015年 He Zitong. All rights reserved.
//

import Foundation

extension UIColor {
    class func yellow() -> UIColor {
        return UIColor(red: 232.0/255.0, green: 255.0/255.0, blue: 140.0/255.0, alpha: 1.0)
    }
    class func green() -> UIColor {
        return UIColor(red: 140.0/255.0, green: 255.0/255.0, blue: 150.0/255.0, alpha: 1.0)
    }
    class func purple() -> UIColor {
        return UIColor(red: 150.0/255.0, green: 140.0/255.0, blue: 255.0/255.0, alpha: 1.0)
    }
    class func orange() -> UIColor {
        return UIColor(red: 255.0/255.0, green: 152.0/255.0, blue: 117.0/255.0, alpha: 1.0)
    }
    class func pink() -> UIColor {
        return UIColor(red: 252.0/255.0, green: 59.0/255.0, blue: 255.0/255.0, alpha: 1.0)
    }
    class func tabBarNormalColor() -> UIColor {
        return UIColor(red: 254.0/255.0, green: 223.0/255.0, blue: 223.0/255.0, alpha: 1.0)
    }
    class func tabBarSelectedColor() -> UIColor {
        return UIColor(red: 252.0/255.0, green: 32.0/255.0, blue: 32.0/255.0, alpha: 1.0)
    }
    class func topGradientColor() -> UIColor {
        return UIColor(red: 218.0/255.0, green: 67.0/255.0, blue: 67.0/255.0, alpha: 1.0)
    }
    class func bottomGradientColor() -> UIColor {
        return UIColor(red: 250.0/255.0, green: 104.0/255.0, blue: 104.0/255.0, alpha: 1.0)
    }
    class func tabBarColor() -> UIColor {
        return UIColor(red: 247.0/255.0, green: 112.0/255.0, blue: 112.0/255.0, alpha: 1.0)
    }
    class func dateLabelColor() -> UIColor {
        return UIColor(red: 253.0/255.0, green: 207/255.0, blue: 207/255.0, alpha: 1.0)
    }
    class func lineColor() -> UIColor {
        return UIColor(red: 254.0/255.0, green: 170.0/255.0, blue: 170.0/255.0, alpha: 1.0)
    }

}